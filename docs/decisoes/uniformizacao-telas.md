# Alinhamento com o Guia de Uniformização de Telas (SENAI)
...
> Plano de record do **alinhamento** entre o **Guia de Uniformização de Telas (SENAI)** — documento de referência que vale pra todas as equipes, cujo conteúdo essencial está consolidado abaixo (§3–§7) — e o [`PLANO_IMPLEMENTACAO.md`](../PLANO_IMPLEMENTACAO.md) (plano do grupo). Doc transversal: vale pros 4 módulos. Cada integrante constrói o módulo dele a partir daqui.

---

## 1. Objetivo

O Guia chegou depois do plano do grupo e é uma **uniformização mais rígida**: fixa **11 telas obrigatórias** com nomes padronizados, **vocabulário canônico** e diretrizes de design comuns a todas as equipes. Ele conflita com decisões já vivas (nomes de entidade, módulos dos integrantes) e força telas que o grupo tinha como pendência. Este doc reconcilia os dois: trava as decisões de alinhamento, mapeia cada tela exigida pro plano existente, e lista os deltas de banco/UI por dono. **Nenhum código de app nesta rodada** — é planejamento (P0 *Planning First*).
...
| # | Decisão | Detalhe |
|---|---------|---------|
| L1 | **Vocabulário: UI adota o Guia, banco mantém** | "Rotas" e "Usuários" nas labels e nas rotas do front; as tabelas seguem `linha` e `usuario`; a camada `services/` traduz. **Sem migração de renomeação** no banco já entregue. |
...
Guia §1. Status: ✅ existe (mockup+plano) · ⚠️ parcial · ❌ a desenhar.

| # | Tela (Guia) | Rota front | Página React | Dono | Status | Ação de alinhamento |
|---|--------------|-----------|--------------|------|--------|---------------------|
| 1 | `login.html` | `/login` | `auth/Login` | Guilherme | ✅ | — |
| 2 | `cadastro.html` | `/cadastro` | `auth/Cadastro` | Guilherme | ✅ | Tipo de usuário **não** no cadastro público (Guia §1.1) — já é o default `comum`. |
...
## 4. Telas opcionais (Guia §2) → status

| Opcional (Guia) | No plano? | Nota |
|------------------|-----------|------|
| Perfil do usuário | ✅ | `cliente/Perfil` + `PerfilEdicao` (Rafael). |
| Compra de passagens | ❌ | Fora de escopo do plano atual; não obrigatória. |
| Viagens (itinerários) | ⚠️ | Guia permite destacar de Rotas. Ligado ao §9 O3 (modelar `viagem`). |
...
## 5. Perfis (Guia §3) ↔ `cargo.nivel_acesso`

| Perfil (Guia) | `cargo.nivel_acesso` | Acesso |
|----------------|----------------------|--------|
...
Compatível com o PLANO §5. Ponto de atenção: o Guia dá ao **maquinista** o poder de *criar alerta* — garantir que o guard de `/admin/alertas` (ou a ação dentro de notificacoes_alertas) aceite `operacional`, não só `gestao`.

---

## 6. Vocabulário canônico (Guia §5)

| Termo Guia / UI | Tabela (banco) | Label UI | Não usar |
|------------------|----------------|----------|----------|
...
## 7. Diretrizes de design (Guia §4) → onde encostam

| Diretriz Guia | Onde no projeto |
|----------------|-----------------|
...
- **`sensor.segmento`** (O2): Guia §1.3 pede "segmento". Hoje `sensor` tem `localizacao`/`tipo_dado`. Adicionar coluna `segmento` **ou** derivar de `localizacao`.
- **Rotas/viagens** (O3): `linha` não tem trajeto/horários. Guia §1.3/§5 quer viagens programadas na tela de rotas, mas **adia o ajuste de itinerário "pra momento posterior"**. Modelar `viagem` (trem, trajeto, horário, status) quando a tela amadurecer.
...
- [ ] **O3** — `viagem`/itinerário: modelar agora ou adiar (como o próprio Guia adia)? Define o quanto a tela de Rotas entrega na 1ª versão.
