# Telas admin de cadastro — Carga e Usuários
...
- **Data**: `criado_em` vem sem fuso, em UTC. A tela formata com `dataHora()` de `src/lib/utils.ts`, que soma `Z` antes do `new Date()`; sem isso o horário sai 3 h adiantado.
