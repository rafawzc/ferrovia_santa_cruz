...
- `.env` criado antes da API (fase 3)? Ele não tem `DB_HOST=db` e o backend quebra no primeiro acesso ao banco. Copie a linha do `.env.example` (e o `JWT_SECRET` novo, com 32+ bytes).
