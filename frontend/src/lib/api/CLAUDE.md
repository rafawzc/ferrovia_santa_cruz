...
- `marcarErrosDeCampo(erro, form)` (em `client.ts`): `422` do FastAPI → `setError('Valor inválido')` em cada `loc[1]` que existe nos valores do form; devolve `true` se era 422 (a página dá `return`). Único lugar dessa tradução.
